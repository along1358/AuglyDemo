...
public class UpdateManager {
    ...
    public void update(Context context, String url, String newVersion, boolean usePgy) {
        ...
        if (new File(apkPath).exists()) AppUtils.installApp(apkPath);
            ...
            if (usePgy) {
                new DownloadTask().download(ServiceConstant.BASE_URL, url, apkPath, downloadListener);
            } else {
                new DownloadTask().download(ServiceConstant.BASE_URL_XUpdateService, ServiceConstant.URL_PREFIX_DOWNLOAD_APP + url, apkPath, downloadListener);
            }
        }
    }
}
